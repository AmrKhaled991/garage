...
class VendorRepository extends BaseRepository {
  Future<LoadingState<List<Clinic>?>> getClinics({
    String? stateId,
    String? categoryId,
    String? search,
    int? page = 1,
  ...
  Future<LoadingState<List<Doctor>?>> getDoctors({
    String? clinicId,
    String? categoryId,
  }) async {
    return networkHandler.getRequest(
      ...
      query: {
        if (clinicId != null) "place_id": clinicId,
        if (categoryId != null) "subcategory_id": categoryId,
      },
  ...
  Future<LoadingState<Restaurant?>> getVendorDetails(String vendorId) async {
    return networkHandler.getRequest(
      ...
      query: {"with_vendor_categories": "yes"},
      create: () => APIResponse<Restaurant>(create: () => Restaurant()),
    );
  }

  Future<LoadingState<List<VendorWorkingTimes>?>> getVendorWorkingTime(
    String vendorId,
  ) async {
    return networkHandler.getRequest(
      endpoint: "view/place/$vendorId",
      create:
          () => APIListResponse<VendorWorkingTimes>(
            create: () => VendorWorkingTimes(),
          ),
    );
  }
}

...
class NetworkHandler extends GetConnect{
  ...
  static getHeaders(){
    return {
      ...
      "Authorization": "Bearer ${Get.find<PreferenceManager>().getAccessToken}",
  ...
  Future<LoadingState<ResultType>> getRequest<M extends Decodable, ResultType>
      ({required String endpoint, required Create<M> create, Map<String, dynamic>? query}) async {

    var response = await get(endpoint,query: query,headers: getHeaders());
    log("***************GET $endpoint ,query: $query , headers: ${getHeaders().toString()} , url: ${response.request?.url} ***************");
    log("***************GET $endpoint Response (${response.statusCode}) ***************\n${response.body}");

    return handleResponse<M, ResultType>(response, create);
  }

  Future<LoadingState<ResultType>> postRequest<M extends Decodable, ResultType>
    ...
    log("***************POST $endpoint ,body: $body , headers: ${getHeaders().toString()} , url: ${response.request?.url} ***************");
    log("***************POST $endpoint Response (${response.statusCode}) ***************\n${response.body}");

    return handleResponse<M, ResultType>(response, create);
  }

  Future<LoadingState<ResultType>> patchRequest<M extends Decodable, ResultType>
      ({ required String endpoint, required Create<M> create, dynamic body}) async {

    var response = await patch(endpoint, body,headers: getHeaders());
    log("***************PATCH $endpoint ,body: $body , headers: ${getHeaders().toString()} , url: ${response.request?.url} ***************");
    log("***************PATCH $endpoint Response (${response.statusCode}) ***************\n${response.body}");
  ...
  Future<LoadingState<ResultType>> putRequest<M extends Decodable, ResultType>
    ...
    log("***************PUT $endpoint ,body: $body , headers: ${getHeaders().toString()} , url: ${response.request?.url} ***************");
    log("***************PUT $endpoint Response (${response.statusCode}) ***************\n${response.body}");

    return handleResponse<M, ResultType>(response, create);
  }

  Future<LoadingState<ResultType>> deleteRequest<M extends Decodable, ResultType>
      ({required String endpoint, required Create<M> create, Map<String, dynamic>? query}) async {

    var response = await delete(endpoint, query: query,headers: getHeaders());
    log("***************DELETE $endpoint ,query: $query , headers: ${getHeaders().toString()} , url: ${response.request?.url} ***************");
    log("***************DELETE $endpoint Response (${response.statusCode}) ***************\n${response.body}");

    return handleResponse<M, ResultType>(response, create);
  }

  LoadingState<ResultType> handleResponse<M extends Decodable, ResultType>(
      Response? response, Create<M> create) {
    ...
    try {
      ...
      if (response?.isOk == true) {
        ResponseWrapper responseWrapper = ResponseWrapper.init(create: create, data: response?.body);

        return LoadingState.success(
            responseWrapper.response.data as ResultType? ,
            pagination: (responseWrapper.response is APIListResponse)? responseWrapper.response.pagination : null,
